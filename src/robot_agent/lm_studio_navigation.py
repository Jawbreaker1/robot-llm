...
def _base_url(value: str, *, allow_private_lan: bool = False) -> str:
    """Backward-compatible navigation-client wrapper."""

    try:
        return validate_lm_studio_base_url(
            value,
            allow_private_lan=allow_private_lan,
        )
    except LMStudioEndpointError as error:
        raise LMStudioNavigationError(str(error)) from error
...
def _model_id(value: str) -> str:
    ...
    try:
        return validate_lm_studio_model_id(value)
    except LMStudioEndpointError as error:
        raise LMStudioNavigationError(str(error)) from error
